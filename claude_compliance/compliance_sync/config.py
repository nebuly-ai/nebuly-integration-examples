...
class Config:
    ...
    def from_env_and_args(cls, argv: list[str] | None = None) -> Config:
        load_dotenv()
        parser = argparse.ArgumentParser(description="Sync Claude Compliance data to Nebuly")
        parser.add_argument("--from-date", type=str, default=None, help="ISO backfill start date")
        ...
        return cls(
            ...
            compliance_api_key=compliance_api_key,
            compliance_base_url=compliance_base_url.rstrip("/"),
            organization_uuid=organization_uuid,
            compliance_max_requests_per_minute=int(
